...
div#rssconter {
        color: 99999a;border: 1px solid #d2d3d9;text-align: left;height: 29;margin: 5px;
}
]]>
</style>
...
<![CDATA[
function GetXmlData(iurl,divname,lnum,isphoto)
{
   ...
   //创建子节点
	...
	if (RssFeedHttp.readyState == 4 && RssFeedHttp.status==200){
            ...
            for(i = 0; i < rssroot.length; i++){
                ...
                if(rssroot.item(i).getElementsByTagName("pubDate").item(0).text != ''){
                    rstr += " - [" + rssroot.item(i).getElementsByTagName("pubDate").item(0).text + "]";
                }
                if(isphoto=='photo'){
                    if(rssroot.item(i).getElementsByTagName("description").item(0).text != ''){
                        rstr += "<div id=\"rssconter\"><img src=\"" + rssroot.item(i).getElementsByTagName("description").item(0).text + "\" width=\"200\"/></div>";
                    }
                }else{
                    if(rssroot.item(i).getElementsByTagName("description").item(0).text != ''){
                        rstr += "<div id=\"rssconter\">" + rssroot.item(i).getElementsByTagName("description").item(0).text + "</div>";
                    }
                }
                rstr += "</li>";
            }
    ...
    <xsl:if test="type=1 or type=2 or type=3">
        <div id="itembody">
            <div id="itemtitle">
                <table width="100%">
                    <tr valign="middle">
                        <td width="30"><img src="../../images/jiaodian_biao.gif" border="0" onclick="new Element.toggle('item_{title}')" /></td>
                        <td><font color="white"><xsl:value-of select="title"/></font></td>
                        <td align="right"><a href="{link}" target="_blank"><img src="../../images/Rss.gif" border="0" align="absmiddle" /></a></td>
                    </tr>
                </table>
            </div>
            <div id="item_{title}">loading...</div>
...
<script type="text/javascript">
    ...
    <xsl:for-each select="body/MyBlog/Blogitem">
        <xsl:if test="type='rss' or type='diary' or type='music' or type='book' or type='photo'">
            GetXmlData('<xsl:value-of select="link"/>','<xsl:value-of select="title"/>',<xsl:value-of select="listnum"/>,'<xsl:value-of select="type"/>');
        </xsl:if>
        <xsl:if test="type=1 or type=2 or type=3">
            GetXmlData('<xsl:value-of select="link"/>','<xsl:value-of select="title"/>',<xsl:value-of select="listnum"/>,'text');
        </xsl:if>
   </xsl:for-each>
</script>
</body>
